...
func (js *SecurityJobSummary) GenerateMarkdownFromFiles(dataFilePaths []string) (markdown string, err error) {
	data, err := loadContent(dataFilePaths)
	if err != nil {
		return "", fmt.Errorf("failed while creating security section markdown: %w", err)
	}
	return GenerateSecuritySectionMarkdown(getSectionData(data, Curation))
}

func getSectionData(data []formats.SummaryResults, section SecuritySummarySection) (sectionData []formats.SummaryResults) {
	for _, summary := range data {
		scans := []formats.ScanSummaryResult{}
		if summary.Section == section {
			sectionData = append(sectionData, summary)
		}
	}
	return
}
