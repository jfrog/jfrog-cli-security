...
func getDockerScanTestResults(unique bool) (*results.SecurityCommandResults, validations.ValidationParams) {
	...
	dockerImageTarget.JasResults.AddApplicabilityScanResults(0,
		&sarif.Run{
			...
			Results:     []*sarif.Result{validations.CreateDummyApplicableResults("CVE-2024-6119", formats.Location{File: "file://" + filepath.Join("usr", "local", "bin", "node"), StartLine: 1, StartColumn: 1})},
		},
	)
	// Secrets scan results
