...
import (
	...
	"github.com/jfrog/jfrog-cli-security/sca/technologies/java/maven"
...
// func BuildDependencyTree(depTreeParams DepTreeParams, tech techutils.Technology) ([]*xrayUtils.GraphNode, map[string]*dependencytree.DepTreeNode, error) {
// 	if tech == techutils.Maven {
// 		return maven.buildMavenDependencyTree(&depTreeParams)
// 	}
...
type moduleDepTree struct {
	...
	Nodes map[string]dependencytree.DepTreeNode `json:"nodes"`
}
...
func getGraphFromDepTree(outputFilePaths string) (depsGraph []*xrayUtils.GraphNode, uniqueDepsMap map[string]*dependencytree.DepTreeNode, err error) {
	modules, err := parseDepTreeFiles(outputFilePaths)
	if err != nil {
		return
	}
	uniqueDepsMap = map[string]*dependencytree.DepTreeNode{}
...
func GetModuleTreeAndDependencies(module *moduleDepTree) (*xrayUtils.GraphNode, map[string]*dependencytree.DepTreeNode) {
	moduleTreeMap := make(map[string]dependencytree.DepTreeNode)
	...
	for depName, dependency := range moduleDeps {
		...
		moduleTreeMap[dependencyId] = dependencytree.DepTreeNode{
			Classifier: dependency.Classifier,
			Types:      dependency.Types,
			Children:   childrenList,
		}
	}
	return dependencytree.BuildXrayDependencyTree(moduleTreeMap, GavPackageTypeIdentifier+module.Root)
}
