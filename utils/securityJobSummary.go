...
const (
	...
	Docker   SecuritySummarySection = "Docker Scans"
...
type ScanCommandResultSummary struct {
	...
	Summary formats.SummaryResults `json:"summary"`
}

// Record the security command outputs
func RecordSecurityCommandSummary(content ScanCommandResultSummary, args ...string) (err error) {
	...
	if index := getDataIndexFromSection(content.ResultType); index != "" {
		return recordIndexData(manager, content, index, args...)
	}
...
func recordIndexData(manager *commandsummary.CommandSummary, content ScanCommandResultSummary, index commandsummary.Index, args ...string) (err error) {
	switch index {
		case commandsummary.BinariesScan:
			// Save the Binary scan result for each artifact separately (key is path)
			for _, scan := range content.Summary.Scans {
				err = errors.Join(err, manager.RecordWithIndex(newScanCommandResultSummary(content.ResultType, scan), index, scan.Target))
			}
		default:
			...
			manager.RecordWithIndex(content, index, args...)
	}
	return
}

func newScanCommandResultSummary(resultType SecuritySummarySection, scans ...formats.ScanSummaryResult) ScanCommandResultSummary {
	return ScanCommandResultSummary{ResultType: resultType, Summary: formats.SummaryResults{Scans: scans}}
}

func loadContent(dataFiles []string, filterSections ...SecuritySummarySection) ([]formats.SummaryResults, error) {
	data := []formats.SummaryResults{}
	for _, dataFilePath := range dataFiles {
		...
		if err := commandsummary.UnmarshalFromFilePath(dataFilePath, &cmdResults); err != nil {
			return nil, fmt.Errorf("failed while Unmarshal '%s': %w", dataFilePath, err)
		}
...
func (js *SecurityJobSummary) GenerateMarkdownFromFiles(dataFilePaths []string) (markdown string, err error) {
	curationData, err := loadContent(dataFilePaths, Curation)
	if err != nil {
		return
	}
...
func GenerateSecuritySectionMarkdown(data []formats.SummaryResults) (markdown string, err error) {
	if len(data) == 0 {
		return
	}
	// Create the markdown content
	markdown += "#### Curation Audit\n| Audit Summary | Project name | Audit Details |\n|--------|--------|---------|"
	for i := range data {
		for _, summary := range data[i].Scans {
			status := SuccessSvg
			details := ""
			if summary.HasIssues() {
				status = FailedSvg
				details = fmt.Sprintf("<pre>%s</pre>", getCurationDetailsString(summary))
			}
			markdown += fmt.Sprintf("\n| %s | %s | %s |", status , summary.Target, details)
		}
	}
	return
}
...
type blockedPackageByType struct {
	BlockedType  string
	BlockedSummary formats.SummaryCount
}

func getCurationDetailsString(summary formats.ScanSummaryResult) (content string) {
	if summary.CuratedPackages == nil {
		return
	}
	content += fmt.Sprintf("Total Number of resolved packages: <b>%d</b>", summary.CuratedPackages.GetTotalPackages())
	content += fmt.Sprintf("<br>🟢 Approved packages: <b>%d</b>", summary.CuratedPackages.Approved)
	blockedPackages := summary.CuratedPackages.Blocked.GetCountOfKeys(false)
	content += fmt.Sprintf("<br>🔴 Blocked packages: <b>%d</b>", blockedPackages)
	...
	for blockTypeName, blockTypeValue := range summary.CuratedPackages.Blocked {
		blocked = append(blocked, blockedPackageByType{BlockedType: blockTypeName, BlockedSummary: blockTypeValue})
	}
	...
	for _, blockStruct := range blocked {
		content += fmt.Sprintf("<br><details><summary>%s (%d)</summary>%s</details>", blockStruct.BlockedType, len(blockStruct.BlockedSummary), blockStruct.BlockedSummary)
	}
	return
}

func getBlockedPackages(blockedSummary formats.SummaryCount) string {
	content := ""
	for blockedPackage, _ := range blockedSummary {
		content += fmt.Sprintf("<br>📦 %s", blockedPackage)
	}
	return content
}
...
type DynamicMarkdownGenerator struct {
	...
	dataFiles []string
	content []formats.SummaryResults
}
...
func (mg *DynamicMarkdownGenerator) loadContentFromFiles() (err error) {
	...
	mg.content, err = loadContent(mg.dataFiles)
	return
}

func (mg *DynamicMarkdownGenerator) GetViolations() (content string, err error) {
	summary := formats.GetViolationSummaries(mg.content...)
	if summary == nil {
		content = "<pre>No watch is defined</pre>"
		return
	}
	violationDetails, err := generateResultsMarkdown(mg.index, true, &summary.ScanVulnerabilitiesSummary)
	if err != nil {
		return
	}
	watches := "watch"
	...
	content += fmt.Sprintf("<pre>%s</pre><br>%s", watches, violationDetails)
	return
}

func (mg *DynamicMarkdownGenerator) GetVulnerabilities() (content string, err error) {
	return generateResultsMarkdown(mg.index, false, formats.GetVulnerabilitiesSummaries(mg.content...))
}

func generateResultsMarkdown(index commandsummary.Index, violations bool, content *formats.ScanVulnerabilitiesSummary) (markdown string, err error) {
	if !content.HasIssues() {
		markdown = fmt.Sprintf("<pre>%s</pre>", getCenteredSvgWithText(SuccessSvg, "No security issues found"))
		return
	}
	markdown += fmt.Sprintf("%d Security Issues: %s<br>%s", content.GetTotalIssueCount(), getResultsTypesSummaryString(content), getJfrogUrl(index, content))
	details := content.GetSummaryDetails()
	for _, severity := range getSummarySortedKeysToDisplay(maps.Keys(details)...) {
		if severityMarkdown, e := getSeverityMarkdown(severity, details); e != nil {
			err = fmt.Errorf("failed to get severity markdown: %w", e)
			return
		} else {
			markdown += severityMarkdown
		}
	}
	markdown = fmt.Sprintf("<pre>%s</pre>", markdown)
	return
}

func getJfrogUrl(index commandsummary.Index, content *formats.ScanVulnerabilitiesSummary) string {
	var moreInfoUrl string
	switch index {
	case commandsummary.BinariesScan:
		moreInfoUrl = getJfrogBinaryScanDetailsUrl(jasutils.GetJfrogUrl(), content.GetScanIds()...)
	case commandsummary.DockerScan:
		moreInfoUrl = getJfrogBinaryScanDetailsUrl(jasutils.GetJfrogUrl(), content.GetScanIds()...)
	case commandsummary.BuildScan:
		moreInfoUrl = getJfrogBuildScanDetailsUrl(jasutils.GetJfrogUrl(), content.GetRepo(), content.GetBuildName(), content.GetBuildNumbers()...)
	}
	return getLinkTag(moreInfoUrl, "See the results of the scan in JFrog")
}

func getSeverityMarkdown(severity string, details formats.TwoLevelSummaryCount) (markdown string, err error) {
	if severitySvg, e := getSeveritySvg(severity); e != nil {
		err = fmt.Errorf("failed to get severity svg: %w", e)
		return
	} else {
		if summary, ok := details[severity]; ok {
			markdown += fmt.Sprintf("<br>%s", getCenteredSvgWithText(severitySvg, fmt.Sprintf("%d %s %s", summary.GetTotal(), severity, summary, getSeverityStatusesCountString(summary))))
		}
	}
}
...
func getSeveritySvg(severityStr string) (svg string, err error) {
	severity, err := severityutils.ParseSeverity(severityStr, false)
	if err != nil {
		return
	}
...
func getLinkTag(url, text string) string {
	return fmt.Sprintf("<a href=\"%s\">%s</a>", url, text)
}

func getCenteredSvgWithText(svg, text string) (markdown string) {
	return fmt.Sprintf("<div style=\"display: flex; align-items: center; text-align: center\">%s %s</div>", svg, text)
}

func getResultsTypesSummaryString(summary *formats.ScanVulnerabilitiesSummary) (content string) {
	return
}

func getJfrogBinaryScanDetailsUrl(jfrogUrl string, scanIds ...string) string {
	baseUrl := fmt.Sprintf("%s/ui/onDemandScanning", jfrogUrl)
	if len(scanIds) == 1 {
...
func getJfrogBuildScanDetailsUrl(jfrogUrl, repo, buildName string, buildNumbers string) string {
	baseUrl := fmt.Sprintf("%s/ui/scans-list/builds-scans/%s/scan-descendants", jfrogUrl, buildName, repo)
	if len(buildNumbers) == 1 {
		return fmt.Sprintf("/%s&package_id=build&build_number=%s", baseUrl, buildNumbers)
	}
	return fmt.Sprintf("%s?build_repository=%s", baseUrl)
}
...
func RecordSecurityCommandOutput(content ScanCommandSummaryResult) (err error) {
	...
	if index := getDataIndexFromSection(content.Section); index != "" {
		return manager.RecordWithIndex(content, index, getIndexArgs(index, content.Results)...)
	}
	content.WorkingDirectory = wd
	return manager.Record(content)
}

// Generate the Security section (Curation, maybe total summary)
