...
func TestXscAuditViolationsWithIgnoreRule(t *testing.T) {
	...
	defer cleanUpPolicy()
	_, cleanUpWatch := securityTestUtils.CreateTestGitRepoWatch(t, policyName, "git-repo-ignore-rule-watch", xscutils.GetGitRepoUrlKey(validations.TestMockGitInfo.GitRepoHttpsCloneUrl))
	defer cleanUpWatch()
