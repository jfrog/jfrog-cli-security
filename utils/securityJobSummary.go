	...
	switch index {
		case commandsummary.BinariesScan:
			...
			for _, scan := range content.Summary.Scans {
				err = errors.Join(err, manager.RecordWithIndex(content, index, scan.Target))
			}
		default:
...
func loadContent(dataFiles []string) ([]formats.SummaryResults, error) {
