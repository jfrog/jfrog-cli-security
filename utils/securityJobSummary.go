...
const (
	...
	OtherStatus SeverityStatus = "%d Other"
)
...
func (s SeverityStatus) Format(count int) string {
	status := fmt.Sprintf(string(s), count)
	switch s {
	case ApplicableStatus:
		return RedColor.Format(status)
	case OtherStatus:
		return OrangeColor.Format(status)
	case NotApplicableStatus:
		return GreenColor.Format(status)
	default:
		return status
	}
}
...
func NewBuildScanArgs(serverDetails *config.ServerDetails, buildName, buildNumber string) *ResultSummaryArgs {
	return &ResultSummaryArgs{BaseJfrogUrl: serverDetails.Url, BuildName: buildName, BuildNumbers: []string{buildNumber}}
}

func NewDockerScanArgs(serverDetails *config.ServerDetails, dockerImage string) *ResultSummaryArgs {
	return &ResultSummaryArgs{BaseJfrogUrl: serverDetails.Url, DockerImage: dockerImage}
}

func NewBinaryScanArgs(serverDetails *config.ServerDetails) *ResultSummaryArgs {
	return &ResultSummaryArgs{BaseJfrogUrl: serverDetails.Url}
}

func (rsa ResultSummaryArgs) ToArgs(index commandsummary.Index) []string {
	args := []string{}
	return args
}
...
func recordIndexData(manager *commandsummary.CommandSummary, content ScanCommandResultSummary, index commandsummary.Index) (err error) {
	...
	default:
		...
		manager.RecordWithIndex(content, index, content.Args.ToArgs(index)...)
	}
	return
}
...
func (mg *DynamicMarkdownGenerator) GetViolations() (content string, _ error) {
	scanIds, summary := formats.GetViolationSummaries(mg.content...)
	if summary == nil {
		content = PreFormat.Format("No watch is defined")
		return
	}
	resultsMarkdown := generateResultsMarkdown(mg.index, true, getJfrogUrl(mg.index, mg.args, scanIds, mg.extendedView), &summary.ScanResultSummary)
...
func (mg *DynamicMarkdownGenerator) GetVulnerabilities() (content string, err error) {
	scanIds, showVulnerabilities, summary := formats.GetVulnerabilitiesSummaries(mg.content...)
	if !showVulnerabilities  {
		// We are in violation mode and vulnerabilities are not requested (no info to show)
		return
	}
	content = generateResultsMarkdown(mg.index, false, getJfrogUrl(mg.index, mg.args, scanIds, mg.extendedView), summary)
	return 
}

func getJfrogUrl(index commandsummary.Index, args ResultSummaryArgs, scanIds []string, extendedView bool) string {
	if !extendedView {
		return ""
	}
	...
	switch index {
	case commandsummary.BinariesScan:
		moreInfoUrl = getJfrogBinaryScanDetailsUrl(args.BaseJfrogUrl, scanIds...)
	case commandsummary.DockerScan:
		moreInfoUrl = getJfrogBinaryScanDetailsUrl(args.BaseJfrogUrl, scanIds...)
	case commandsummary.BuildScan:
		moreInfoUrl = getJfrogBuildScanDetailsUrl(args.BaseJfrogUrl, args.BuildName, args.BuildNumbers...)
	}
	return Link.Format(moreInfoUrl, "See the results of the scan in JFrog")
}
...
func getJfrogBuildScanDetailsUrl(jfrogUrl, buildName string, buildNumbers ...string) string {
...
func generateSeverityStatusesCountString(displayData map[SeverityStatus]int) string {
	...
	if count, ok := displayData[OtherStatus]; ok {
		display = append(display, OtherStatus.Format(count))
	}
