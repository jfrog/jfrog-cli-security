...
func (auditCmd *AuditCommand) Run() (err error) {
	return auditCmd.ProcessResultsAndOutput(RunAudit(auditCmd.CreateAuditParams(nil)))
}

func (auditCmd *AuditCommand) CreateAuditParams(gitInfo *services.XscGitInfoContext) *AuditParams {
	msi := auditCmd.multiScanId
	if msi == "" {
		var event *xscservices.XscAnalyticsGeneralEvent
		if gitInfo != nil {
			log.Debug("MultiScanId wasn't provided, sending xsc analytics event with git context.")
			event = auditCmd.analyticsMetricsService.CreateGeneralEventWithGitContext(xscservices.CliProduct, xscservices.CliEventType, gitInfo)
		} else {
			log.Debug("MultiScanId wasn't provided, sending xsc analytics event.")
			event = auditCmd.analyticsMetricsService.CreateGeneralEvent(xscservices.CliProduct, xscservices.CliEventType)
		}
		// Don't set `auditCmd.multiScanId = msi` to act as a flag that event was generated by the command for the final event information update.
		if msi = auditCmd.analyticsMetricsService.AddGeneralEvent(event); msi == "" {
			log.Debug("Failed to add general event and get MultiScanId.")
...
func (auditCmd *AuditCommand) ProcessResultsAndOutput(auditResults *results.SecurityCommandResults) (err error) {
	if auditCmd.multiScanId == "" && auditResults.MultiScanId != "" {
		// The User didn't provide a multiScanId and we handled the xsc analytics events so we need to update the final event information.
		...
		log.Debug(fmt.Sprintf("Command skips updating the final event information as MSI was provided by the user: %s", auditCmd.multiScanId))
	}
...
func RunJasScans(auditParallelRunner *utils.SecurityParallelRunner, auditParams *AuditParams, scanResults *results.SecurityCommandResults, jfrogAppsConfig *jfrogappsconfig.JFrogAppsConfig) (jasScanner *jas.JasScanner, generalError error) {
	...
	jasScanner, err = jas.CreateJasScanner(serverDetails, scanResults.SecretValidation, auditParams.minSeverityFilter, jas.GetAnalyzerManagerXscEnvVars(auditParams.multiScanId, scanResults.GetTechnologies()...), auditParams.Exclusions()...)
...
func initAuditCmdResults(params *AuditParams) (cmdResults *results.SecurityCommandResults) {
	...
	// XSC info
	if _, params.xscVersion, err = xsc.CreateXscServiceManagerAndGetVersion(serverDetails); err != nil {
		log.Debug(fmt.Sprintf("Failed to get XSC version. %s", err.Error()))
	} else {
		cmdResults.SetXscVersion(params.xscVersion)
	}
	cmdResults.SetMultiScanId(params.multiScanId)
