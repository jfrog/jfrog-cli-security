...
func TestValidateInputFile(t *testing.T) {
	...
	cdx := cyclonedx.NewBOMWithSpecVersion(cyclonedx.SpecVersion1_6)
...
func TestGenerateURLFromPath(t *testing.T) {
	...
	assert.NoError(t, utils.SaveCdxContentToFile(validCdxFilePath, cdx))
	expectedSha, err := calculateFileSHA256(validCdxFilePath)
	assert.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := generateURLFromPath(baseUrl, tt.repoPath, tt.filePath)
			assert.NoError(t, err)
