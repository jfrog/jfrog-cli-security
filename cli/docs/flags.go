...
var flagsMap = map[string]components.Flag{
	...
	ServerId:    components.NewStringFlag(ServerId, "Server ID configured using the 'jf c add' command."),
	...
	Watches:       components.NewStringFlag(Watches, "A comma-separated(,) list of Xray watches, to enable Xray to determine violations accordingly. The command accepts this option only if the --repo-path and --repo-path options are not provided. If none of the three options are provided, the command will show all known vulnerabilities."),
