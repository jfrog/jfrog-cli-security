...
func TestCalcShouldReportEvents(t *testing.T) {
	...
	msiCallback := tests.SetEnvWithCallbackAndAssert(t, jas.JfMsiEnvVariable, "")
	defer msiCallback()
	...
	err := os.Setenv(jas.JfMsiEnvVariable, "")
...
func getDummyContentForGeneralEvent(withJas, withErr bool) *results.ScanCommandResults {
	...
	// scaResults := []resultutils.ScaScanResult{{XrayResults: []services.ScanResponse{{Vulnerabilities: vulnerabilities}}}}

	cmdResults := results.NewCommandResults("", true)
	scanResults := cmdResults.NewScanResults("target")
	...
	if withErr {
		scanResults.Errors = errors.New("an error")
	}
