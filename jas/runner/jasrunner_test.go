...
func TestGetExtendedScanResults_AnalyzerManagerDoesntExist(t *testing.T) {
	auditParallelRunnerForTest := utils.CreateSecurityParallelRunner(cliutils.Threads)
	...
	err = RunJasScannersAndSetResults(auditParallelRunnerForTest, scanResults.ExtendedScanResults, scanResults.GetScaScannedTechnologies(), scanResults.GetScaScansXrayResults(), []string{"issueId_1_direct_dependency", "issueId_2_direct_dependency"}, &jas.FakeServerDetails, nil, false, "", applicability.ApplicabilityScannerType, secrets.SecretsScannerType, auditParallelRunnerForTest.AddErrorToChan)
	// Expect error:
	assert.Error(t, err)
}
...
func TestGetExtendedScanResults_ServerNotValid(t *testing.T) {
	...
	err := RunJasScannersAndSetResults(auditParallelRunnerForTest, scanResults.ExtendedScanResults, scanResults.GetScaScannedTechnologies(), scanResults.GetScaScansXrayResults(), []string{"issueId_1_direct_dependency", "issueId_2_direct_dependency"}, nil, nil, false, "", applicability.ApplicabilityScannerType, secrets.SecretsScannerType, auditParallelRunnerForTest.AddErrorToChan)
	assert.NoError(t, err)
}
...
func TestGetExtendedScanResults_AnalyzerManagerReturnsError(t *testing.T) {
	...
	scanResults := &utils.Results{ScaResults: []*utils.ScaScanResult{{Technology: techutils.Yarn, XrayResults: jas.FakeBasicXrayResults}}, ExtendedScanResults: &utils.ExtendedScanResults{}}
	err := RunJasScannersAndSetResults(auditParallelRunnerForTest, scanResults.ExtendedScanResults, scanResults.GetScaScannedTechnologies(), scanResults.GetScaScansXrayResults(), []string{"issueId_2_direct_dependency", "issueId_1_direct_dependency"}, &jas.FakeServerDetails, nil, false, "", applicability.ApplicabilityScannerType, secrets.SecretsScannerType, auditParallelRunnerForTest.AddErrorToChan)
