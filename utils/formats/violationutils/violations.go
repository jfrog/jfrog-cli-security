...
func (vs *Violations) HasViolations() bool {
	return len(vs.Sca) > 0 || len(vs.License) > 0 || len(vs.OpRisk) > 0 || len(vs.Secrets) > 0 || len(vs.Iac) > 0 || len(vs.Sast) > 0 || len(vs.Services) > 0
}

func (vs *Violations) Count() int {
	return len(vs.Sca) + len(vs.License) + len(vs.OpRisk) + len(vs.Secrets) + len(vs.Iac) + len(vs.Sast) + len(vs.Services)
}
