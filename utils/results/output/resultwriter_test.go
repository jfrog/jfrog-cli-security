...
func TestConvertXrayScanToSimpleJson(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			cmdResults.NewScanResults("target").NewScaScanResults(&tc.result)
			...
			output, err := ConvertXrayScanToSimpleJson(cmdResults, false, tc.includeLicenses, true, tc.allowedLicenses)
			if assert.NoError(t, err) {
