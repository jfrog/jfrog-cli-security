...
func (auditCmd *AuditCommand) CreateAuditParams() *AuditParams {
	...
	if msi == "" {
		log.Debug("MultiScanId wasn't provided, sending xsc analytics events.")
		// Don't set `auditCmd.multiScanId = msi` to act as a flag that information was generated by the command for the final event information update.
		msi = auditCmd.analyticsMetricsService.AddGeneralEvent(auditCmd.analyticsMetricsService.CreateGeneralEvent(xscservices.CliProduct, xscservices.CliEventType))
	}
