...
// job's workflow YAML for Subpath/Parent metadata, filters out anything unrelated to this
// workflow (e.g. a leftover cache entry from a previous job on a reused self-hosted runner),
// decides a curation outcome per remaining action, prints and records the report, and returns
// an error if any action was Rejected.
